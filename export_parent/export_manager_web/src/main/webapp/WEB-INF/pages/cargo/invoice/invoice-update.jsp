        ...
        <form id="addInvoiceForm" action="/cargo/invoice/edit.do" method="post">
            <div class="panel panel-default">
                <div class="row data-type" style="margin: 0px">
                    <input type="text" name="invoiceId" id="invoiceId" value="${invoice.invoiceId}" style="display: none;">
                        ...
                        <table id="dataList" class="table table-bordered table-striped table-hover dataTable">
                            ...
                            <tbody>
                                    <tr class="odd" onmouseover="this.className='highlight'" onmouseout="this.className='odd'" >
                                        <td>${Shipping.shippingOrderId}</td>
                                        <td align="center">
                                            <c:if test="${Shipping.orderType eq 'SEA'}">
                                                海运
                                            </c:if>
                                            <c:if test="${Shipping.orderType eq 'AIR'}">
                                                空运
                                            </c:if>
                                        </td>
                                        <td>${Shipping.shipper}</td>
                                        <td>${Shipping.consignee}</td>
                                        <td>${Shipping.notifyParty}</td>
                                        <td>${Shipping.lcNo}</td>
                                        <td>${Shipping.portOfLoading}</td>
                                        <td>${Shipping.portOfTrans}</td>
                                        <td>${Shipping.portOfDischarge}</td>
                                        <td><fmt:formatDate value="${Shipping.loadingDate}" pattern="yyyy-MM-dd"/></td>
                                        <td><fmt:formatDate value="${Shipping.limitDate}" pattern="yyyy-MM-dd"/></td>
                                        <td>${Shipping.specialCondition}</td>
                                        <td>${Shipping.freight}</td>
                                        <td>${Shipping.checkBy}</td>
                                        <td><c:if test="${Shipping.shippingState==0}">草稿</c:if>
                                            <c:if test="${Shipping.shippingState==1}"><font color="green">已上报</font></c:if></td>
                                        <td>
                                            <a href="${ctx }/cargo/shippingOrder/toView.do?id=${Shipping.shippingOrderId}">[查看]</a>
                                            <a href="/cargo/shippingOrder/exportPdf.do?id=${Shipping.shippingOrderId}">[下载]</a>
                                        </td>
                                    </tr>
